...
import com.triline.billionlights.utils.OFFER_SENT
import com.triline.billionlights.utils.REQUEST_RECEIVED
...
class MyOffersPagerAdapter(fm: FragmentManager, private val context: Context) :
        ...
        when (position) {
            0 -> {
                if (offerReceivedFragment == null) {
                    offerReceivedFragment = OffersReceivedFragment.newInstance(REQUEST_RECEIVED)
                }
                return offerReceivedFragment!!
            }
            1 -> {
                if (requestSentFragment == null) {
                    requestSentFragment = RequestSentFragment.newInstance(OFFER_SENT)
                }
